...
function effectiveValue(
  ...
  const v = liveLocation?.[field.key];
  return typeof v === "string" ? v : "";
}

export default function LocationForm({ runId, scope, scopeKey }: Props) {
  ...
  const assetEntry = assets?.locations.find((l) => l.name === locationName) ?? null;
  ...
  const handleBlur = useCallback(
    async (field: FieldDef) => {
      ...
      const serverValue = effectiveValue(field, liveLocation, draftFields);
    ...
    [local, liveLocation, draftFields, stageDraftFields, runId, scope, scopeKey],
  );

  if (!liveLocation) {
    return <div className="shot-form-empty">No location loaded.</div>;
  }
    ...
    <div className="shot-form">
      <div className="shot-form-header">
        ...
        <div className="shot-form-meta" style={{ marginTop: 4 }}>
          Apply commits the draft to <code>updateLocation</code>. If a reference image
          is staged, it replaces the canonical reference and frames using this location
          are re-queued.
        </div>
      </div>
      ...
      {FIELDS.map((field) => {
        const value =
          field.key in local ? local[field.key] : effectiveValue(field, liveLocation, draftFields);
