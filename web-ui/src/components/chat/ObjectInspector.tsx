...
export default function ObjectInspector({ runId, scope, scopeKey, messages }: Props) {
  ...
  const previewArtifacts =
    session && isObjectDraft(session.draft) ? session.draft.previewArtifacts : undefined;
  ...
  const resolvedObject = useMemo(() => {
    ...
    return { ...liveObject, ...draftFields };
  }, [liveObject, draftFields]);
  ...
  const proposedField = (key: string): unknown | null => {
    ...
    if (shallowEqual(proposed, liveObject?.[key])) return null;
    return proposed;
  };
  ...
  const pendingScalarFields = useMemo(() => {
    ...
    for (const key of Object.keys(draftFields)) {
      ...
      const proposed = draftFields[key];
      const canonical = liveObject?.[key];
      if (shallowEqual(proposed, canonical)) continue;
      out.push({
        key,
        label: OBJECT_FIELD_LABELS[key] ?? key,
        canonical,
        proposed,
      });
    }
    return out;
  }, [draftFields, liveObject]);

  const objectName = (liveObject?.name as string | undefined) ?? scopeKey;
  const objectAsset = useMemo(
    () => assets?.objects.find((o) => o.name === objectName) ?? null,
    [assets, objectName],
  );
  const objectItems = useMemo(
    () => findItemsByKey(queues, `asset:object:${objectName}`),
    [queues, objectName],
  );
  const latestObjectItem = useMemo(() => pickLatest(objectItems), [objectItems]);
  const isObjectInFlight =
    latestObjectItem != null &&
    (latestObjectItem.status === "pending" || latestObjectItem.status === "in_progress");
  const isObjectFailed =
    latestObjectItem != null && latestObjectItem.status === "failed";

  const handleCopyJson = () => {
    if (!resolvedObject) return;
    void navigator.clipboard?.writeText(JSON.stringify(resolvedObject, null, 2));
  };
  ...
  const sections: ContextInspectorSection[] = [
      ...
      render: () => (
        <div className="shot-inspector-current-outputs">
          <div className="shot-inspector-current-output">
            ...
            {objectAsset?.imagePath && activeRunId ? (
              <div className="shot-inspector-current-thumb">
                <img
                  src={mediaUrl(activeRunId, objectAsset.imagePath)}
                ...
                {isObjectInFlight && (
                  <div className="shot-inspector-current-overlay">
                    <span className="badge badge-in_progress">
                      {progressLabel(
                        latestObjectItem ? itemProgress[latestObjectItem.id] : undefined,
                      )}
                    </span>
                  </div>
                )}
      ...
      render: () => (
        <>
          <dl className="inspector-dl">
            <dt>Name</dt>
            <dd>{(liveObject?.name as string | undefined) ?? "—"}</dd>
            {proposedName !== null && (
              <dd>
            ...
            <dd style={{ whiteSpace: "pre-wrap" }}>
              {(liveObject?.visualDescription as string | undefined) ?? "—"}
            </dd>
      ...
      render: () => {
        ...
        return empty ? (
          ...
          <pre className="inspector-pre">
            {JSON.stringify({ objectFields: draftFields, pendingReferenceImage }, null, 2)}
          </pre>
        );
      },
    },
    ...
    },
    {
      ...
      render: () => (
        downstream.length === 0 ? (
          <div className="shot-inspector-empty">No active items reference this object.</div>
        ) : (
      ...
      render: () => (
        resolvedObject ? (
          <div className="shot-inspector-json-wrap">
            ...
            <pre className="shot-inspector-json">{JSON.stringify(resolvedObject, null, 2)}</pre>
          </div>
        ) : (
          <div className="shot-inspector-empty">Live object not loaded yet.</div>
        )
      ),
    },
  ];
