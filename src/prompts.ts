...
HARD CEILING — MAX 36 WORDS OF DIALOGUE PER SHOT (non-negotiable): 15s is Grok's absolute max shot duration. At 2.5 words/second plus 0.5s buffer, that allows at most ~36 words of dialogue in a single shot (36/2.5 + 0.5 = 14.9s). If a character's speech would exceed ~36 words in one shot, you MUST split it across two or more consecutive shots with the same speaker. Use a cut to a different composition (e.g. over-the-shoulder → close-up, or two_shot → close_up) or the same framing with a time cut. Break the speech at a natural pause — a sentence boundary, a comma, or a breath. Do NOT cram a long speech into one shot: the dialogue floor will exceed 15s, get clamped to 15s, and the TTS audio will be cut off or talk over itself. This rule is stricter than the "don't split continuous dialogue" guidance below — at >36 words, splitting is REQUIRED.
...
- Dialogue-heavy (10-15s): Required when dialogue word count demands it (~25-36 words). Give the speaker the time they need, up to the 36-word / 15s hard ceiling. If speech exceeds 36 words, split across consecutive shots.
...
4. Distribute dialogue across shots. "Dialogue" includes ALL spoken content: character speech, narration, voiceover, inner monologue, and any text that should be heard by the viewer. If the scene description mentions a voice, narrator, or internal thought, include it as dialogue in the appropriate shot. Shot durations MUST be whole numbers (integers), minimum 2 seconds. Never use fractional durations like 1.5 or 2.5. CRITICAL: calculate the minimum duration for each shot from its dialogue word count at ~2.5 words/second, then add 0.5s buffer. The shot's durationSeconds must NEVER be less than this minimum. Example: 12 words of dialogue = 12/2.5 + 0.5 = 5.3s → round up to 6s minimum. HARD CEILING: no single shot may contain more than ~36 words of dialogue (14.9s floor), because 15s is the max shot duration. If a character's speech exceeds 36 words, split it across consecutive shots with the same speaker — cut to a different composition (e.g. over_the_shoulder → close_up) at a natural pause (sentence boundary, comma, breath). This splitting is MANDATORY, not optional: a single shot with >36 words will have its dialogue clamped to 15s and the generated audio will cut off.
...
14. Write videoPrompt as a COMPLETE, SELF-CONTAINED description of what happens in the shot from the video model's perspective. This is the primary direction sent to the video model — it must contain EVERYTHING the video model needs in natural prose:
   ...
   - Dialogue with natural visual attribution must follow Rule 15 exactly: include the exact dialogue text in straight double-quotes, attributed to a visual descriptor or voiceover as required.
   ...
   - Camera movement
   Write it as a flowing paragraph of direction, not a list. The existing structured fields (dialogue, speaker, soundEffects, cameraDirection) are still required — they're used for subtitles, scripts, scope chats, and metadata. But videoPrompt is what goes to the video model and must be self-contained.
...
15. MANDATORY DIALOGUE QUOTING IN videoPrompt: If a shot's dialogue field is non-empty, the EXACT text of that dialogue field MUST appear verbatim inside straight double-quotes (") somewhere in the videoPrompt. No paraphrasing, no truncation, no summarizing, no ellipses, no curly quotes, and no single quotes. The video model generates the shot's audio from videoPrompt; there is no separate TTS pass. Dialogue that is not quoted in videoPrompt will not be heard by the audience.
   Speaker attribution depends on the speaker field:
   - Character speaker listed in charactersPresent: attribute the quoted line to a short visual descriptor, not the character name. Example: the woman in the dark coat says: "...". The character's lips will move and the video model will lip-sync the visible speaker.
   - narrator, voiceover, or any non-character speaker: attribute the quoted line as voiceover. Example: Voiceover (no character speaks on camera): "...". The videoPrompt MUST also explicitly state that no character is speaking on camera, or the video model may lip-sync the audio to a random visible face.
   - inner monologue: attribute the quoted line as inner-monologue voiceover. Example: Inner monologue voiceover (no character speaks on camera): "...". The videoPrompt MUST explicitly state that no character is speaking on camera.
   - Empty dialogue field: this rule does not apply; leave speaker empty.
   The dialogue field remains the canonical metadata source for subtitles, scripts, scope chats, and tooling. The verbatim text is duplicated into videoPrompt only so the video model's audio generator says it.
   BAD videoPrompt: The tall masked woman holds the bulletin and looks down at it. She begins reading in a warm, resonant voice, then abruptly catches herself — she clears her throat and restarts in a harsh, ugly croak, deliberately distorting her voice. She reads steadily in the forced ugly tone, her masked face angled down at the papers. Camera slowly pushes in. Fluorescent hum overhead.
   GOOD videoPrompt: The tall masked woman holds the bulletin and looks down at it. She begins reading in a warm, resonant voice: "Harrison Bergeron, age fourteen, has just escaped from jail, where he was held on suspicion of plotting to overthrow the government." She abruptly catches herself mid-sentence, clears her throat, and restarts in a harsh, ugly croak, deliberately distorting her voice as she repeats the line. Camera slowly pushes in. Fluorescent hum overhead.
16. NO UNNAMED HUMANS: NEVER mention ANY human figure who is not listed in charactersPresent for that shot. This includes waiters, servers, bartenders, hosts, background diners, couples at nearby tables, passersby, staff, or any unnamed person. The video model will hallucinate random people into the scene. If the story involves a waiter serving food, describe the food appearing on the table or show only disembodied hands — never describe a waiter as a person. If the scene is in a restaurant, describe empty tables, not tables with diners. Do NOT use phrases like 'other diners', 'background patrons', 'couples at nearby tables', 'the crowd', etc. The ONLY humans visible in any shot must be those listed in charactersPresent.
17. NEVER include music, jazz, songs, score, soundtrack, or any musical terms in videoPrompt or soundEffects. The video model generates audio and will produce music if prompted. Only reference non-musical ambient sounds (chatter, footsteps, wind, clinking, engine noise, etc). Music is added in post-production.
18. FACE VISIBILITY RULE: When planning a shot with continuousFromPrevious=true, consider whether every character whose face needs to be visible in this shot also had their face clearly visible in the previous shot. If the previous shot was an over-the-shoulder, behind-the-subject, or insert shot where a character's face was NOT visible (back of head, side profile, or off-screen), and the current shot needs to show that character's face, set continuousFromPrevious=false so a fresh start frame is generated with the correct reference image. The video model cannot generate a correct face from scratch — it will hallucinate a random face that doesn't match the character. Only faces that were clearly visible in the previous shot's frame will be rendered correctly in a continuous shot.
19. MID-SHOT FACE REVEAL BAN: The video model CANNOT generate a correct face that is not already visible in the start frame. If a character is facing away, shown from behind, or otherwise has their face hidden in the startFramePrompt, the videoPrompt MUST NOT describe them turning around, looking over their shoulder toward camera, or otherwise revealing their face during the shot. The resulting face will be randomly generated and will not match the character. If you need to show a character's face after a behind-the-subject shot, create a NEW shot with continuousFromPrevious=false so the start frame is generated fresh from reference images with the character's face visible. Similarly, if a character enters the shot during the video (walks into frame), do NOT describe their face becoming visible — either start the shot with them already in frame (face visible in startFramePrompt) or keep them out of frame entirely and show them in the next shot.
20. DIALOGUE PLACEMENT: In videoPrompt, describe all visual blocking and action BEFORE dialogue. The video model handles sequential actions well, so structure each shot as: physical movement and gestures first, then spoken lines. Example: The man holds both hands out in a calming gesture, looking off-screen right. He says: "Now, now. I'll ask the Microvac right now." — not: He says: "Now, now. I'll ask the Microvac right now." while holding his hands out.
21. ACTION COMPLEXITY: Limit each shot to at most 2 simultaneously active characters performing independent actions. If more characters need to act independently, split into separate shots — e.g., a reaction shot of the children, then cut to parents watching. Wide establishing shots can show multiple characters but only 1-2 should have complex independent motion.
22. MOTION INTENSITY: Use adverbs of degree to specify how actions should be performed — "slowly reaches", "rapidly turns", "gently places", "violently shakes", "powerfully stands". The video model responds strongly to motion intensity words and produces better results with explicit degree modifiers.
23. PACING AND TEMPO: The overall tempo should be FAST. Think trailer editing, not art house cinema. Every shot must have visible action, dialogue, or meaningful visual change happening throughout its duration. Apply these rules aggressively:
...
24. SCREENS AND DISPLAYS: If a shot includes a computer, phone, tablet, TV, monitor, or any device with a screen, add this instruction to BOTH the startFramePrompt and videoPrompt: "Images appear only on the screen. Not the back." This prevents the video model from hallucinating content on the wrong surface of the device. Apply this whenever any screen-bearing object is visible in the shot — whether held by a character, sitting on a desk, mounted on a wall, etc.`;
